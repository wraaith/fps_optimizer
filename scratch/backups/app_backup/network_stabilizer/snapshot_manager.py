...
class SnapshotManager:
    ...
    def _load_state(self) -> Dict[str, Any]:
        ...
        by preserving the corrupt file with a timestamp and re-initializing valid schema.
        """
        if not os.path.exists(self.state_file):
        ...
        try:
            with open(self.state_file, "r", encoding="utf-8") as f:
                data = json.load(f)
                if isinstance(data, dict) and "baseline" in data and "changes" in data:
                    return data
                raise ValueError("Incomplete schema in state file.")
        except Exception as e:
            # Handle corrupt state file by making a backup and restoring default schema
            print(f"[SnapshotManager] Warning: State file corrupted ({e}). Recovering...", file=sys.stderr)
            ...
            try:
                ...
                pass
            clean_schema = self._get_empty_schema()
            self._save_state_atomic(clean_schema)
            return clean_schema
    ...
    def is_baseline_present(self) -> bool:
        ...
        return bool(state.get("baseline", {}).get("created_at"))
    ...
    def log_change(
        ...
        state.setdefault("changes", []).append(record)
        self._save_state_atomic(state)
        return change_id
